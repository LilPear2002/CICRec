...
class DataHandlerSequential:
    def __init__(self):
        ...
        predir = './datasets/sequential/{}/'.format(dataset_name)
    ...
    def _seq_aug(self, user_seqs):
        """Sequential augmentation: split sequences into multiple training samples"""
    ...
    def load_data(self):
        ...
        # Get num_workers from config (default to 0 for Windows compatibility)
        num_workers = configs['train'].get('num_workers', 0)
        
        self.test_dataloader = data.DataLoader(
            tst_data, batch_size=configs['test']['batch_size'], shuffle=False, num_workers=num_workers)
        self.train_dataloader = data.DataLoader(
            trn_data, batch_size=configs['train']['batch_size'], shuffle=True, num_workers=num_workers)
